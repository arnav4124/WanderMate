...
\begin{enumerate}
  ...
  \item \textbf{Strategy adapters:} Three adapter classes, each wrapping a specific API behind a common normalised output:
    \begin{itemize}
      \item \code{PlacesAdapter} (\code{services/placesAdapter.js}) --- wraps Google Places (Text Search, Nearby Search, Find Place, Place Details). Normalises responses into the shared POI shape.
      \item \code{ORSAdapter} (\code{services/orsAdapter.js}) --- wraps ORS POI and routing endpoints. Implements both simple routing (\code{/v2/directions}) and multi-stop optimization (\code{/optimization}).
