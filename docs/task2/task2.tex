...
WanderMate is a collaborative mobile travel planner built as a React~Native (Expo) frontend communicating with a Node.js/Express backend, backed by MongoDB for persistent storage and Firebase Realtime Database for real-time event propagation. The system integrates multiple external APIs --- Google Places, OpenRouteService (ORS), and a disabled Overpass adapter --- all proxied through a backend API Gateway that acts as a secret vault and central policy enforcement point.
...
\caption{C4 Context Diagram: WanderMate system boundary showing the two internal components (Mobile App, API Gateway), five external systems (Google Places, ORS, Firebase Auth, Firebase RTDB, MongoDB Atlas), and two actor types (Traveller, Collaborator). All external API credentials reside exclusively on the backend.}
...
5 & \textbf{External API Providers} & Third-party services whose APIs WanderMate consumes, each with usage quotas, SLAs, and terms of service that constrain the system's architecture. & Google Cloud (Places API), OpenRouteService, Firebase / Google Cloud Platform. \\
...
\rowcolor{tablealtbg}
C6 & \textbf{API Quota Compliance} & The system must operate within the free-tier quotas of all external APIs at launch: Google Places ($\leq$ 5,000 req/month), ORS ($\leq$ 2,000 req/day). Quota violations would incur financial cost and potentially break the service. & Supervisor, API Providers $\cdot$ \reqid{NFR-06} \\
...
\begin{enumerate}
  ...
  \item \textbf{API Gateway} (Node.js + Express) --- single backend entry point with middleware stack: \code{helmet()} security headers, \code{authenticate()} Firebase JWT verification (\code{middleware/auth.js}), \code{generalLimiter} (100~req/15~min), and \code{cacheMiddleware} response cache (\code{middleware/cache.js}).
  \item \textbf{POI \& Routing Service} (\code{TravelService} Facade) --- encapsulates \code{PlacesAdapter} (Google Places), \code{ORSAdapter} (OpenRouteService), and the disabled \code{OverpassAdapter} behind a unified interface with \code{node-cache} caching (30~min search, 24~hr details, 1~hr routes).
...
\textbf{View:} WanderMate uses a \textbf{dual-database architecture}: MongoDB as the authoritative store for all domain data, and Firebase RTDB as a thin event layer carrying notification payloads only.

\textbf{MongoDB data model} (Mongoose ODM):
\begin{itemize}
  \item \textbf{Trip} (\code{models/Trip.js}) --- nested document: \code{name}, \code{destination}, \code{startDate}, \code{endDate}, \code{owner} (Firebase UID), \code{collaborators[]} (UIDs), \code{days[]} $\rightarrow$ \code{stops[]} with \code{name}, \code{lat}, \code{lng}, \code{category}, \code{order}, \code{cost}, etc. Indexes: \code{(owner, createdAt)}, \code{(collaborators)}, \code{(isPublished, publishedAt)}.
  \item \textbf{Expense} (\code{models/Expense.js}) --- separate collection, referenced by \code{trip} ObjectId. Fields: \code{description}, \code{amount}, \code{category} (accommodation | food | transport | activities | other), \code{date}, \code{paidBy}.
...
\textbf{Firebase RTDB paths} (thin event layer):
\begin{itemize}
  \item \code{trips/\{tripId\}} --- carries \code{\{updatedAt, lastEditedBy\}} on every mutation. Written by \code{TripObserver} event handler (\code{services/tripObserver.js}).
...
\begin{itemize}
  \item MongoDB is the \emph{single source of truth} for all complex queries (follower graphs, expense aggregations, feed pagination). Firebase is intentionally kept thin --- it does not store full documents.
...
\caption{Information Viewpoint --- MongoDB data model (Mongoose schemas) and Firebase RTDB event paths. The four MongoDB collections (Trip, Expense, FeedPost, User) serve as the authoritative data store; the three Firebase paths carry lightweight event notifications only.}
...
MongoDB & MongoDB Atlas (cloud) & Connection via \code{MONGODB\_URI} in \code{.env}. Configured in \code{config/database.js} with Mongoose ODM (v9.4). Indexes pre-defined in model schemas for query performance. \\
...
The following Architecture Decision Records document major design decisions using the \textbf{Nygard ADR template} (Title, Status, Context, Decision, Consequences). Each ADR is grounded in specific code artefacts and requirements from the WanderMate repository.

% -----------------------------------------------------------
\subsection{ADR-001: Use Firebase Realtime Database as an Event Bus for Collaboration (Not as a Primary Data Store)}
% -----------------------------------------------------------

\begin{tcolorbox}[adrbox]

\textbf{\large ADR-001: Use Firebase RTDB as a Thin Event Layer}
...
We will use \textbf{MongoDB as the single source of truth} for all domain data and \textbf{Firebase RTDB as a thin event bus} that carries only notification payloads (timestamps and metadata), not full documents.
...
\begin{itemize}
  ...
  \item The \code{TripObserver} writes lightweight payloads to Firebase paths:
    \begin{itemize}
      \item \code{trips/\{tripId\}} $\gets$ \code{\{name, destination, owner, updatedAt: Date.now()\}}
      \item \code{budgets/\{tripId\}/lastUpdate} $\gets$ timestamp
      \item \code{feed/\{postId\}} $\gets$ \code{\{author, tripName, createdAt: Date.now()\}}
    \end{itemize}
  \item Frontend stores subscribe via \code{onValue} listeners (\code{tripStore.subscribeTripUpdates()}, \code{budgetStore.subscribeBudgetUpdates()}) and, upon receiving a notification, re-fetch the full authoritative data from MongoDB via the API.
\end{itemize}
...
\begin{itemize}
  ...
  \item Firebase costs scale only with notification volume, not with data storage.
...
\textbf{Negative:}
\begin{itemize}
  \item The re-fetch pattern (notification $\to$ API call $\to$ MongoDB read) adds one HTTP round-trip compared to a Firebase-primary approach where data would be pushed directly.
...
WanderMate consumes three external APIs with sensitive credentials:
\begin{itemize}
...
\begin{enumerate}
  ...
  \item Applies a middleware stack on every request:
    \begin{itemize}
      ...
      \item \code{cors(\{origin: '*'\})} --- allows cross-origin requests from the Expo development server.
  ...
  \item Proxies external API calls exclusively through the \code{TravelService} Facade, which instantiates adapters with keys from \code{process.env}:
    \begin{itemize}
      \item \code{this.places = new PlacesAdapter(process.env.GOOGLE\_PLACES\_API\_KEY)} (\code{travelService.js}, line~11).
      \item \code{this.ors = new ORSAdapter(process.env.ORS\_API\_KEY)} (\code{travelService.js}, line~12).
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Facade interface:} Five public methods --- \code{searchPOI()}, \code{searchByCategory()}, \code{getPlaceDetails()}, \code{getRoute()}, \code{optimizeRoute()} --- abstract away all provider and caching logic.
  \item \textbf{Strategy adapters:} Three adapter classes, each wrapping a specific API behind a common normalised output:
    \begin{itemize}
      \item \code{PlacesAdapter} (\code{services/placesAdapter.js}) --- 168 lines, wraps Google Places (Text Search, Nearby Search, Find Place, Place Details). Normalises responses via \code{\_normalize(place)} with category detection (\code{\_detectCategory(types)}).
      \item \code{ORSAdapter} (\code{services/orsAdapter.js}) --- 240 lines, wraps ORS POI and routing endpoints. Normalises POIs via \code{\_normalizePOI(feature)}. Implements both simple routing (\code{/v2/directions}) and multi-stop optimization (\code{/optimization}).
      \item \code{OverpassAdapter} (\code{services/overpassAdapter.js}) --- 115 lines, currently disabled. Uses mirror failover across multiple Overpass servers.
    \end{itemize}
  \item \textbf{Singleton:} A module-level instance (\code{let travelServiceInstance = null}) with a \code{getTravelService()} factory function ensures one instance is shared across all route modules.
  \item \textbf{Caching:} A single \code{node-cache} instance inside the Facade applies per-method TTLs: 30~minutes for searches (\code{poi\_search\_*}), 24~hours for details (\code{poi\_details\_*}), 1~hour for routes (\code{route\_*}).
...
\begin{itemize}
  ...
  \item \textbf{Centralized caching:} One \code{node-cache} instance serves all providers; no duplicate cache logic. Cache key generation uses method name + parameters (\code{poi\_search\_\$\{query\}\_\$\{lat\}\_\$\{lng\}}).
...
\begin{enumerate}
  ...
  \begin{verbatim}
    export const localCache = {
      ...
      async set(key, data) { /* stores {data, timestamp} */ },
      async clear(key) { /* removes entry */ }
    };
  \end{verbatim}
  ...
  \end{verbatim}
  When a write operation fails (network error), the store catches the exception and enqueues the mutation. The queue is flushed on reconnect via the Profile screen's ``Sync Offline Changes'' button, replaying operations in timestamp order.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Manual sync trigger:} The queue does not auto-flush on reconnect in the current implementation; users must navigate to the Profile screen and tap ``Sync Offline Changes.'' This is a usability gap acknowledged for future improvement.
  ...
  \item \textbf{Deleted resource handling:} If a collaborator deletes a trip or stop while another user is offline, the offline user's queued mutations targeting that resource will receive 404 responses during flush. The current \code{flush()} implementation does not distinguish 404s from transient errors --- it stops replaying on the first failure, leaving remaining operations in the queue. Robust handling would require detecting 404/410 responses and discarding the corresponding stale operations.
...
001 & Firebase RTDB as thin event bus, MongoDB as source of truth & \adrstatus{Accepted} & C7 (Real-time), C3 (Offline) & NFR-03 (sync $<$500ms), FR-20/21 (collaboration) \\
...
\begin{tcolorbox}[notebox]
  \textbf{Inter-ADR dependencies.} ADR-001 (Firebase as event bus) depends on ADR-002 (API Gateway) because the gateway is the component that writes events to Firebase after persisting to MongoDB. ADR-003 (Facade pattern) is implemented within the gateway defined by ADR-002. ADR-004 (offline-first) complements ADR-001 by providing a client-side persistence layer that bridges connectivity gaps.
\end{tcolorbox}
...
\begin{enumerate}
  \item Using Firebase RTDB as a thin notification layer while MongoDB remains the authoritative data store (ADR-001).
