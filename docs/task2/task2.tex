...
\begin{itemize}
  \item \code{trips/\{tripId\}} --- on \code{TRIP\_CREATED}: carries \code{\{name, destination, owner, updatedAt\}} (metadata only); on \code{TRIP\_UPDATED}: carries the full sanitised trip document with \code{lastEditedBy} appended, enabling direct client-side application without a MongoDB re-fetch. Written by \code{TripObserver} event handler (\code{services/tripObserver.js}).
...
\begin{itemize}
  ...
  \item The \code{TripObserver} writes payloads to Firebase paths:
    \begin{itemize}
      \item \code{trips/\{tripId\}} $\gets$ on \code{TRIP\_CREATED}: \code{\{name, destination, owner, updatedAt\}}; on \code{TRIP\_UPDATED}: the full sanitised trip document (\code{JSON.parse(JSON.stringify(trip))}) with \code{lastEditedBy} and \code{updatedAt} appended.
      \item \code{budgets/\{tripId\}/lastUpdate} $\gets$ timestamp
      \item \code{feed/\{postId\}} $\gets$ \code{\{author, tripName, createdAt: Date.now()\}}
    \end{itemize}
  \item Frontend stores subscribe via \code{onValue} listeners (\code{tripStore.subscribeTripUpdates()}, \code{budgetStore.subscribeBudgetUpdates()}). For trip updates the \code{tripStore} listener applies the received Firebase document directly to local state using a last-write-wins (LWW) timestamp comparison (\code{if (remote.updatedAt >= local.updatedAt)}) --- no additional HTTP call to MongoDB is made.
\end{itemize}
...
\textbf{Negative:}
\begin{itemize}
  \item For \code{TRIP\_UPDATED} events the full trip document is written to Firebase and applied directly on the client --- eliminating any extra HTTP round-trip. However, this makes Firebase carry heavier write payloads than a pure thin-event design, and means trip data in Firebase may temporarily diverge from MongoDB if the sanitisation step produces a stale read.
