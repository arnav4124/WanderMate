...
\begin{enumerate}
  ...
  \item \textbf{Fallback chain:} \code{searchPOI()} attempts ORS first, catches failures or empty results, and falls through to Google Places:
  \begin{verbatim}
...
\begin{itemize}
  ...
  \item \textbf{Graceful degradation:} The fallback chain (ORS $\to$ Google Places $\to$ empty) is invisible to consumers, satisfying NFR-08.
\end{itemize}
...
\begin{itemize}
  ...
  \item The fallback chain introduces latency when the primary provider fails. Two sequential API calls (ORS $\to$ Google Places) are made before returning results, though caching mitigates this for repeated queries.
\end{itemize}
...
\caption{ADR-003 Component Diagram: the TravelService Facade receives calls from route handlers, checks the \code{node-cache}, and dispatches to the fallback chain (ORS $\to$ Google Places $\to$ empty array). All adapters normalise responses to a common POI interface. The OverpassAdapter is implemented but currently disabled.}
