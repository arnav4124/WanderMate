...
Users shall be able to search for POIs by keyword and category across 13 supported categories: hotel, restaurant, landmark, activity, transport, museum, park, shopping, nightlife, medical, grocery, finance, and other. The primary search is served by the Google Places Text Search and Nearby Search APIs; when Google Places returns zero results, the system falls back to the ORS POI endpoint. Results are sorted by Haversine distance from the map centre and capped at 15 (text search) or 20 (category browsing) for relevancy. &
\high \\
