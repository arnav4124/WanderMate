...
\begin{tcolorbox}[notebox]
  \textbf{Architectural significance.} The dual-API design uses OpenRouteService (ORS) as the primary POI provider (for search and category browsing) with the Google Places API as a quota-sensitive fallback that supplies richer enrichment data (photos, ratings, opening hours) when ORS returns no results. An Overpass API adapter was initially developed but is currently disabled; the Facade pattern allows it to be re-enabled without changes to calling code. A server-side \texttt{node-cache} caching layer controls free-tier quota consumption. Nominatim (OpenStreetMap) is used as the geocoding provider for destination resolution, with a strict 5\,000\,ms timeout and Google Places Text Search as a fallback.
\end{tcolorbox}
...
\reqid{FR-05} & POI Discovery (Proximity Sort) &
Users shall be able to intelligently search for POIs contextually bound to their map viewport and active trip. Search results are computationally sorted via a backend Haversine formula calculation for exact proximity, capping at 15 items for queries and 20 for explicit category selections. &
\high \\

\rowcolor{tablealtbg}
\reqid{FR-06} & POI Discovery (Categories) &
The system shall support 13+ rich POI categories (e.g., Shopping, Nightlife, Medical, Grocery, Finance) individually mapped to appropriate ORS tags and Google Places descriptors, rendering colour-coded interactive map markers with embedded details. &
\high \\
...
\begin{tcolorbox}[notebox]
  \textbf{Architectural significance.} The social feed is backed by a dedicated \texttt{FeedPost} MongoDB collection with indexes on \texttt{createdAt} and \texttt{likeCount} for efficient pagination and sorting. Feed posts are denormalised snapshots (trip name, destination, budget summary, stop count) rather than live references, ensuring feed rendering is independent of trip mutations. Offset-based pagination is implemented via page + \texttt{skip/limit} queries.
\end{tcolorbox}
...
Users shall be able to send follow requests, accept/deny incoming requests, and remove connections. The ``Following'' feed shall display posts only from users in the authenticated user's \texttt{following[]} list. The feed screen provides a \texttt{SegmentedButtons} toggle between ``Discover'' (all posts) and ``Following'' modes. &
\high \\
...
Users shall be able to invite other registered users to co-edit a trip. The \texttt{POST /api/trips/:id/collaborators} endpoint adds a user's Firebase UID to the trip's \texttt{collaborators[]} array only when the user is already connected (follower/following), then broadcasts the update to Firebase. Changes by any collaborator propagate to all others in real time via Firebase \texttt{onValue} listeners in \texttt{tripStore.subscribeTripUpdates()}. &
\high \\
...
The MongoDB data schema shall be designed to support horizontal sharding by user ID as a \emph{future scalability path} to accommodate growth beyond 10\,000 registered users without schema migration. Indexes on \texttt{owner}, \texttt{collaborators}, \texttt{firebaseUid}, and \texttt{createdAt} are defined to support this. Note: sharding is not available on MongoDB Atlas free/shared tiers and would require migration to a dedicated cluster; at launch the system operates on a single replica set. &
10\,000$+$ users (future path) \\

\reqid{NFR-06} & Scalability &
The system shall operate within free-tier API quotas at launch: Google Places ($\leq$ 5\,000 req/month), ORS ($\leq$ 2\,000 req/day), achieved through server-side \texttt{node-cache} caching and per-user \texttt{express-rate-limit} rate limiting (100 general req/15\,min, 50 external API req/hour). \emph{Note:} \texttt{express-rate-limit} uses an in-memory store by default; counters reset on process restart and are not shared across instances. This is acceptable for the current single-instance deployment but would require a Redis-backed store before horizontal scaling. &
...
\begin{tcolorbox}[notebox]
  \textbf{Architectural impact.} NFR-07 mandates the AsyncStorage-backed \texttt{localCache} and \texttt{syncQueue} modules on the client. NFR-08 is implemented via the Strategy pattern in \texttt{TravelService}: ORS is queried first; if it returns no results, the search method falls back to Google Places without any change to the calling code or UI layer.
\end{tcolorbox}
...
If the ORS POI endpoint returns no results, POI search shall fall back to Google Places seamlessly; if Google Places is also unavailable, an empty result set shall be returned with no user-facing crash or blank screen. The \texttt{TravelService.searchPOI()} method wraps each provider call in a try/catch and silently falls through to the next provider. &
...
In production deployments, all API communication between the mobile client and the backend shall use HTTPS; plaintext HTTP connections shall be rejected. Local development may use HTTP on trusted local networks. &
...
WanderMate is decomposed into \textbf{six primary logical subsystems} and one cross-cutting infrastructure layer. In the current implementation, these are modular boundaries within a single frontend app and a single backend service (not separately deployed services). They communicate through well-defined interfaces and map directly to one or more functional requirement groups defined in Section~1.
...
Mobile Client, MongoDB, Firebase Admin SDK, Google Places API, ORS \\

\textbf{POI \& Routing Service}\newline(TravelService Facade) &
Encapsulates the dual-API POI strategy (ORS as primary, Google Places as quota-sensitive fallback) and ORS route optimisation behind a single \texttt{TravelService} interface. Implements response caching and provider fallback. An Overpass adapter exists but is currently disabled. &
...
The mobile client is the sole user-facing component of WanderMate. It is built with React Native using the Expo managed workflow (SDK 54), targeting Android and web. Navigation is provided by Expo Router with a bottom tab bar containing five primary screens: \textbf{My Trips} (trips list with create/set-active actions), \textbf{Explore} (context-aware POI search + integrated map with auto-centering + itinerary panel via \texttt{@gorhom/bottom-sheet}; implemented in \texttt{explore2.tsx}), \textbf{Feed} (social feed with discover/following toggle), \textbf{Budget} (expense tracking with category breakdowns), and \textbf{Profile} (user stats, offline sync trigger, sign-out).

The interactive map is rendered by embedding Leaflet 1.9.4 inside a \texttt{WebView} component. The map uses OpenStreetMap tiles and displays numbered markers with category-coloured borders across 13 supported POI categories, each with dedicated icons and colour coding. A bidirectional JavaScript bridge passes itinerary data into the WebView and receives user interactions (pin clicks, stop selection) out via \texttt{postMessage}/\texttt{onMessage}. The map intelligently auto-recenters (via \texttt{mapCenterTick}) whenever the active trip changes: if the trip has existing stops, the viewport snaps to the first itinerary stop; otherwise, the trip's destination name is geocoded via the backend's \texttt{/api/poi/geocode} endpoint. Route polylines from ORS are rendered as GeoJSON overlays; when ORS route data is unavailable, a dashed straight-line fallback is drawn.
...
\begin{itemize}
  ...
  \item \texttt{/api/poi} --- POI search, category browsing, and geocoding (proxied via TravelService)
...
This module implements the \textbf{Facade} and \textbf{Strategy} patterns. \texttt{TravelService} is instantiated as a singleton and exposes five methods to the gateway layer: \texttt{searchPOI(query, lat, lng, radius)}, \texttt{searchByCategory(category, lat, lng, radius)} (both of which now sort outputs by a calculated Haversine proximity distance capped at fixed thresholds), \texttt{geocode(query)} (implementing a strict 5000ms Nominatim timeout before gracefully failing over to Google Places), \texttt{getRoute(coordinates, profile)}, and \texttt{optimizeRoute(coordinates)} (multi-stop visit-order optimization via ORS VROOM solver, now protected by a 35s frontend timeout ceiling). Internally, two active adapters implement provider-specific API logic:
This module implements the \textbf{Facade} and \textbf{Strategy} patterns. \texttt{TravelService} is instantiated as a singleton and exposes six methods to the gateway layer: \texttt{searchPOI(query, lat, lng, radius)}, \texttt{searchByCategory(category, lat, lng, radius)}, \texttt{getPlaceDetails(placeId, name, lat, lng)}, \texttt{getRoute(coordinates, profile)}, \texttt{optimizeRoute(coordinates)} (multi-stop visit-order optimization via ORS VROOM solver), and \texttt{geocode(query)} (Nominatim with Google Places fallback). Internally, two active adapters implement provider-specific API logic:

\begin{itemize}
  \item \textbf{ORSAdapter} --- primary POI provider and routing engine. Queries the ORS \texttt{/pois} endpoint for POI search and the \texttt{/v2/directions} and \texttt{/optimization} endpoints for routing and multi-stop visit-order optimisation (VROOM solver). Category mappings use ORS category IDs (e.g., shopping: 500, nightlife: 260/560, medical: 460, finance: 430).
  \item \textbf{PlacesAdapter} --- quota-sensitive fallback POI provider. Queries Google Places Text Search, Nearby Search, Find Place, and Place Details APIs, supplying richer enrichment data (photos, ratings, opening hours, price levels). Supports 13 category types mapped to Google Places types (e.g., \texttt{shopping\_mall}, \texttt{night\_club}, \texttt{hospital}, \texttt{supermarket}, \texttt{bank}). Quota: 5\,000 req/month on free tier.
...
\textbf{Proximity sorting and result capping:} All POI search methods compute the Haversine great-circle distance between the map centre and each result using a dedicated \texttt{\_calculateDistance()} method, sort results by ascending distance, and cap the output (15 results for text search, 20 for category search) to ensure relevancy and prevent UI clutter.

\textbf{Geocoding:} The \texttt{geocode()} method resolves destination names to coordinates using Nominatim (OpenStreetMap) as the primary geocoder with a strict 5\,000\,ms timeout and \texttt{User-Agent: WanderMate/1.0} header. On failure or empty results, it falls back to Google Places Text Search. The route is exposed at \texttt{GET /api/poi/geocode?q=query}.

A shared \texttt{node-cache} instance sits in front of all adapter methods. Cache TTLs are: 30 minutes for search results, 24 hours for place details, and 1 hour for route calculations. If ORS returns no results, \texttt{TravelService} falls back to Google Places, providing richer enrichment data; if both fail, an empty array is returned, satisfying NFR-08 without any change to the calling code.
...
The feed endpoint supports offset-based pagination via page + \texttt{skip/limit} with two modes: ``Discover'' (all posts sorted by \texttt{createdAt}) and ``Following'' (posts by followed users only). The \texttt{feedStore} on the client manages pagination state and implements infinite scroll via \texttt{onEndReached}.
