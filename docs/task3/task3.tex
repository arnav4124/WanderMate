...
\begin{enumerate}
  ...
  \item The \code{TripObserver} event handler catches the event and writes a \textbf{lightweight notification payload} --- containing only metadata (\code{\{updatedAt, lastEditedBy, name, destination\}}) --- to \code{trips/\{tripId\}} in \textbf{Firebase Realtime Database}. Importantly, the full trip document is \emph{not} written to Firebase.
