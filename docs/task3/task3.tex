...
WanderMate is a social, collaborative mobile travel planner built as a React Native (Expo) frontend communicating with a Node.js/Express backend, backed by MongoDB for persistent storage and Firebase Realtime Database for real-time event propagation. The system integrates multiple external APIs --- OpenRouteService (primary POI search and routing), Google Places (quota-sensitive POI fallback with richer metadata), Nominatim (geocoding with fallback to Google Places), and Overpass/OSM (disabled fallback) --- all proxied through a backend API Gateway.
...
\rowcolor{tablealtbg}
\textbf{L2: Middle\-ware} & Express middleware \code{cacheMiddleware} & Per-route response cache with configurable TTL (default 1~hr) & \code{backend/src/middleware/cache.js} \\

\textbf{L3: Service} & In-process \code{node-cache} inside TravelService & POI search: 30~min, Place details: 24~hr, Routes: 1~hr & \code{backend/src/services/travelService.js} \\
...
\caption{Multi-layer caching flow: requests traverse L1 (client-side AsyncStorage), L2 (Express response middleware), and L3 (TravelService \code{node-cache}) before reaching external APIs. The numbering follows the actual request path order.}
...
\begin{tcolorbox}[notebox]
  \textbf{Limitation:} \code{express-rate-limit} uses an in-memory store by default. Rate limit counters are reset on process restart and are not shared across multiple Node.js instances. This is acceptable for the current single-instance deployment (see Task~2, VP3), but would require migration to a Redis-backed store (e.g., \code{rate-limit-redis}) before horizontal scaling.
\end{tcolorbox}
...
\begin{itemize}
  ...
  \item \textbf{NFR-08 (Reliability / API Failure)} --- When the ORS POI endpoint returns no results, POI search falls back to Google Places with zero user-facing crashes.
...
\begin{enumerate}
  \item \textbf{Server-side Provider Fallback}: The \code{TravelService.searchPOI()} method wraps the ORS POI API call in a \code{try/catch}. On failure or empty results, it silently falls through to the Google Places endpoint:
...
\caption{Graceful degradation chains. Left: server-side POI provider fallback (ORS $\to$ Google Places $\to$ empty). Right: client-side offline fallback (API $\to$ AsyncStorage cache $\to$ error message).}
...
The \code{TravelService} Facade exposes six high-level methods (\code{searchPOI}, \code{searchByCategory}, \code{getPlaceDetails}, \code{getRoute}, \code{optimizeRoute}, \code{geocode}) that abstract away all provider-specific logic, including proximity sorting and geocoding. Internally, it delegates to interchangeable adapter classes (\code{PlacesAdapter}, \code{ORSAdapter}, and the disabled \code{OverpassAdapter}), each of which implements the same normalised interface but targets a different external API. The Facade also encapsulates caching, fallback orchestration, Haversine distance sorting with result capping (15 for text queries, 20 for categories), and error handling. The \code{geocode()} method provides destination-to-coordinate resolution using Nominatim (5\,000\,ms timeout) with Google Places Text Search as a fallback.
...
\rowcolor{tablealtbg}
Fallback Orchestration & The Facade's \code{searchPOI()} method implements the fallback chain (ORS $\to$ Google Places $\to$ empty). Adapters are unaware of each other; fallback logic is centralised in the Facade. \\
...
\begin{enumerate}
  ...
  \item The \code{TripObserver} event handler catches the event, re-reads the updated \code{Trip} document from MongoDB, serialises it (\code{JSON.parse(JSON.stringify(trip))}) with \code{lastEditedBy} and \code{updatedAt} appended, and writes the \textbf{full sanitised trip document} to \code{trips/\{tripId\}} in \textbf{Firebase Realtime Database}. This eliminates a separate client re-fetch and delivers the authoritative state in a single push.
  \item Firebase pushes the update to all connected clients that have registered \code{onValue} listeners on that path.
  \item \textbf{User B's} \code{tripStore.subscribeTripUpdates()} callback fires with the full trip payload. The store applies the data \textbf{directly to local state} using a last-write-wins (LWW) timestamp comparison (\code{if (remote.updatedAt >= local.updatedAt)}) --- no additional HTTP call to MongoDB is required.
...
T4: Graceful Degradation & Facade + Strategy & The fallback chain (ORS $\to$ Google Places $\to$ empty) is orchestrated inside the Facade. Adapters are unaware of fallback logic. \\
...
\begin{enumerate}
  \item \textbf{Multi-layer Response Caching} ensures sub-2-second response times and free-tier quota compliance through a three-tier cache hierarchy (client $\to$ middleware $\to$ service).
...
The \textbf{Facade + Strategy} pattern (TravelService) unifies multiple external API providers behind a single interface, centralising caching, fallback, proximity sorting, geocoding, and quota management. The \textbf{Observer} pattern (Firebase Realtime Database subscriptions) enables real-time collaborative editing with sub-500ms propagation latency, decoupled publisher--subscriber communication, and automatic resource cleanup.
