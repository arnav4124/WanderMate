...
To provide concrete quantitative evidence for the architectural choices, we measure the following non-functional requirements under controlled conditions.

\subsection{NFR-01: API Response Time (Performance)}

\reqid{NFR-01} requires POI search results to be returned within 2 seconds on a 4G connection.

\begin{longtable}{L{4.0cm} C{2.5cm} C{2.5cm} C{2.5cm}}
\toprule
\rowcolor{tableheaderbg}
\thcell{Scenario} & \thcell{Arch.\ A (Gateway)} & \thcell{Arch.\ B (Direct)} & \thcell{Target} \\
\midrule
\endhead
\bottomrule
\endfoot

Cold POI search (cache miss) & \emph{TBD} & \emph{TBD} & $< 2\,\text{s}$ \\
\rowcolor{tablealtbg}
Warm POI search (cache hit) & \emph{TBD} & \emph{TBD} & $< 2\,\text{s}$ \\
Route optimisation (5 stops) & \emph{TBD} & \emph{TBD} & $< 2\,\text{s}$ \\
\rowcolor{tablealtbg}
Geocode (Nominatim) & \emph{TBD} & \emph{TBD} & $< 2\,\text{s}$ \\
\end{longtable}

\begin{tcolorbox}[notebox]
  \textbf{Note.} Quantitative latency measurements will be populated after benchmark testing against both architectures. The expectation is that Architecture~A will show slightly higher cold-miss latency due to the gateway hop, but \textbf{significantly lower warm latency} due to server-side cache sharing (cache hits skip the external API entirely and return in microseconds from \code{node-cache}).
\end{tcolorbox}

\subsection{NFR-06: API Quota Consumption (Scalability)}

\reqid{NFR-06} requires the system to operate within free-tier quotas: Google Places $\leq$ 5\,000 req/month, ORS $\leq$ 2\,000 req/day.

\begin{longtable}{L{4.0cm} C{2.5cm} C{2.5cm} C{2.5cm}}
\toprule
\rowcolor{tableheaderbg}
\thcell{Scenario (10-user session)} & \thcell{Arch.\ A (Gateway)} & \thcell{Arch.\ B (Direct)} & \thcell{Quota Limit} \\
\midrule
\endhead
\bottomrule
\endfoot

Google Places calls (1 hour) & \emph{TBD} & \emph{TBD} & 5000/month \\
\rowcolor{tablealtbg}
ORS calls (1 hour) & \emph{TBD} & \emph{TBD} & 2000/day \\
Cache hit ratio & \emph{TBD} & \emph{N/A (no server cache)} & -- \\
\end{longtable}

\begin{tcolorbox}[notebox]
  \textbf{Note.} The expected result is that Architecture~A's server-side \code{node-cache} will dramatically reduce outbound API calls by serving identical queries from cache. In Architecture~B, each client maintains its own cache, so the same query from 10 different users generates 10 separate API calls.
\end{tcolorbox}
